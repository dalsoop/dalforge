...
func autoGitWorkflow(dalName string) string {
	...
	// Determine PR skip reason (if any)
	skipReason := prSkipReason(dalName, changes)
	if skipReason != "" {
		log.Printf("[git] PR skip: %s", skipReason)
	}
	...
	// Skip PR if there is a skip reason
	if skipReason != "" {
		run("git", "checkout", "main")
		return fmt.Sprintf("✅ 커밋+푸시 완료 (브랜치: `%s`)\n📋 PR 스킵: %s", branch, skipReason)
	}
...
// prSkipReason returns a human-readable reason to skip PR creation,
// or empty string if a PR should be created.
func prSkipReason(dalName string, changes string) string {
	// 1. Metadata-only changes (.dal/, .claude/) → no PR needed
	if isDalOnlyChanges(changes) {
		return ".dal/.claude/ 전용 변경"
	}

	// 2. Auto-generated files only (go.sum, lock files) → no PR needed
	if isAutoGeneratedOnly(changes) {
		return "자동 생성 파일만 변경 (go.sum 등)"
	}

	// 3. Duplicate PR check — skip if dal already has an open PR
	if reason := hasPendingDALPR(dalName); reason != "" {
		return reason
	}
...
// isDalOnlyChanges checks if all changed files are under .dal/ or .claude/ directories.
func isDalOnlyChanges(porcelainOutput string) bool {
	...
	for _, line := range lines {
		...
		// Skip the status prefix (first 3 chars: 2 status + 1 space)
		if len(line) < 4 {
			continue
		}
		file := line[3:]
		...
		if !strings.HasPrefix(file, ".dal/") && !strings.HasPrefix(file, ".claude/") {
			return false
		}
	}
	return true
...
// hasPendingDALPR checks if the dal already has an open PR.
// Returns a skip reason string if a pending PR exists, empty string otherwise.
func hasPendingDALPR(dalName string) string {
	cmd := exec.Command("gh", "pr", "list", "--state", "open",
		"--json", "number,headRefName", "--limit", "50")
	cmd.Dir = "/workspace"
	out, err := cmd.Output()
	if err != nil {
		return "" // can't check → don't skip
	}
	...
	prefix := fmt.Sprintf("dal/%s/", dalName)
	for _, pr := range prs {
		if strings.HasPrefix(pr.HeadRefName, prefix) {
			return fmt.Sprintf("이미 열린 PR 있음: #%d (%s)", pr.Number, pr.HeadRefName)
		}
	}
	return ""
}

// isAutoGeneratedOnly checks if all changed files are auto-generated
// (e.g., go.sum, package-lock.json, yarn.lock).
func isAutoGeneratedOnly(porcelainOutput string) bool {
