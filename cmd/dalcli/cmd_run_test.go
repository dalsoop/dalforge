...
func TestIsDalOnlyChanges(t *testing.T) {
	tests := []struct {
		...
		{"dal only", " M .dal/data/claims.json\n M .dal/dev/instructions.md", true},
		{"claude only", " M .claude/settings.json", true},
		{"mixed dal and claude", " M .dal/foo.txt\n M .claude/bar.txt", true},
		{"mixed with code", " M .dal/foo.txt\nM  cmd/dalcli/main.go", false},
		{"code only", "M  internal/daemon/daemon.go", false},
		{"new file", "?? newfile.go", false},
		{"new dal file", "?? .dal/new.json", true},
		{"renamed file", "R  old.go -> new.go", false},
		{"renamed within dal", "R  .dal/old.json -> .dal/new.json", true},
		{"empty", "", true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestIsAutoGeneratedOnly(t *testing.T) {
	tests := []struct {
		name  string
		input string
		want  bool
	}{
		{"go.sum only", " M go.sum", true},
		{"package-lock only", " M package-lock.json", true},
		{"go.sum + code", " M go.sum\nM  main.go", false},
		{"go.mod + go.sum", " M go.mod\n M go.sum", false},
		{"multiple lock files", " M go.sum\n M yarn.lock", true},
		{"empty", "", true},
		{"code only", "M  main.go", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := isAutoGeneratedOnly(tt.input)
			if got != tt.want {
				t.Errorf("isAutoGeneratedOnly(%q) = %v, want %v", tt.input, got, tt.want)
			}
		})
	}
}
...
func TestPrSkipReason(t *testing.T) {
	tests := []struct {
		name    string
		dalName string
		changes string
		wantNon bool // true if we expect a non-empty skip reason
	}{
		{"dal only changes", "dev", " M .dal/data/foo.json", true},
		{"auto-gen only", "dev", " M go.sum", true},
		{"code changes", "dev", "M  cmd/dalcli/main.go", false},
		{"mixed dal+code", "dev", " M .dal/foo.txt\nM  main.go", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := prSkipReason(tt.dalName, tt.changes)
			if tt.wantNon && got == "" {
				t.Errorf("expected non-empty skip reason for %q", tt.name)
			}
			if !tt.wantNon && got != "" {
				t.Errorf("expected empty skip reason, got %q", got)
			}
		})
	}
}
