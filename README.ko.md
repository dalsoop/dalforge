<div align="center">
  <h1>dalcenter</h1>
  <p><strong><code>.dal-template</code> 선언을 로컬 런타임과 Proxmox LXC 운영으로 바꾸는 self-hosted orchestration stack</strong></p>
  <p>
...
`dalcenter`는 `.dal-template` 선언을 실제 로컬 실행과 LXC 운영으로 바꾸는 self-hosted orchestration stack입니다. `dalcenter`는 패키지와 스펙을 유통하고, 사용자 레포의 `.dal-template`를 읽어 등록하고 관리합니다.

공식 도메인: `https://dalcenter.com`

## 개요

- **`dalcenter`** — 패키지와 스펙을 유통하고, `.dal-template`를 읽어 등록, export, 실행, provision을 담당하는 self-hosted 허브
- **사용자 레포** — `.dal-template/`를 가진 실제 프로젝트

한 줄로 줄이면:

`.dal-template`가 SSOT이고, `dalcenter`가 배포하고, 실행하고, 관리한다.
...
`dalcenter`는 이걸 `.dal-template` 하나로 묶어서, 로컬 export와 Proxmox LXC까지 이어줍니다.

## 지금 되는 것

`dalcenter`는 현재 `.dal-template`를 실제 운영 흐름으로 연결합니다.

- `catalog search` 기반 dalcenter 클라우드 패키지 조회
...
설계 문서만 있는 상태가 아니라, `.dal-template`에서 로컬 실행과 LXC 운영까지 이어지는 초기 운영 버전입니다.
...
`dalcenter`는 패키지와 스펙을 유통하고, `.dal-template`를 읽어 등록하고 상태를 관리하는 self-hosted 허브입니다.

- 패키지 배포, 스펙/문서 유통, 버전 카탈로그
...
사용자 레포 루트에 위치하는 폴더입니다. `dalcenter` 허브 레포가 아니라, 실제 프로젝트 레포 안에 들어갑니다. 이 폴더가 실행, export, container, agents를 선언하는 SSOT입니다.
...
`.dal-template/templates/claude-dev.cue`에 인형 틀을 정의합니다. 컨테이너 base image, 설치할 패키지, 에이전트, CLI 도구, 스킬, 필요한 시크릿을 선언합니다.
...
1. 사용자 레포의 `.dal-template/dal.cue` 읽기
2. manifest validate
3. skill/hook export
...
`.dal-template/`가 소스(SSOT)이고, 각 에이전트용 설정은 빌드 산출물로 export됩니다.
...
패키지 이름만으로도 dalcenter에서 받아와서 등록할 수 있습니다.
...
- `.dal-template` 없이 수동 설정만 전제하는 런타임
...
- `.dal-template` 선언을 유지해야 하는 대신 재현성과 관리성이 좋아짐

## 대략적인 비교

| 도구 형태 | 기본 모델 | dalcenter 차이점 |
|---|---|---|
| 단순 CLI 설치기 | 패키지 설치 후 끝 | `.dal-template`를 읽고 등록, export, 상태, provision까지 연결 |
| 일반 task runner | 로컬 명령 실행 중심 | 레포 선언과 런타임 상태를 함께 관리 |
| 컨테이너 provision 도구 | 인프라 생성 중심 | skill/hook export와 로컬 실행 흐름까지 같이 다룸 |

## 현재 갭

OpenClaw 같은 더 제품화된 agent stack과 비교하면, dalcenter는 아직 아래가 약합니다.

- agent-facing 진입점을 하나로 묶는 공용 gateway가 없음
- 세션 단위 context compaction 정책이 없음
- `.dal-template` 기반 skill/hook discovery는 되지만 자동 등록 경험이 아직 약함
...
모든 규약은 `dalcenter/dal.spec.cue`에 CUE로 정의되어 있습니다. 이 파일이 dalforge의 근간이며, 모든 도구는 이 스펙을 따릅니다.
